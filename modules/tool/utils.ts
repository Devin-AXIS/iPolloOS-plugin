 ...
 * distAssetsDir: dist/public/iPolloOS-plugins/tools/[filename]
 * move files:
...
export const parsePkg = async (filepath: string, temp: boolean = true) => {
  ...
  const mod = (await import(`${entryPath}?v=${entryStat.size}-${entryStat.mtimeMs}-${Date.now()}`))
    .default as ToolSetType | ToolType;
