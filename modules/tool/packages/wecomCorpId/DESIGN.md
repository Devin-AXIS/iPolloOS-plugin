...
2. 使用 iPolloOS 反向调用框架实现跨线程通信
3. 支持在 iPolloOS 工作流中作为系统工具使用
4. 类型安全、错误处理完善
...
lib/invoke/wecom/getAuthToken.ts
      ↓
iPolloOS API: /api/support/wecom/getAuthToken
      ↓
...
- **类型验证**: Zod
- **通信机制**: iPolloOS Invoke Framework
- **API**: iPolloOS Internal API
...
async function getAuthToken(
  ...
  // 调用 iPolloOS API
  const url = new URL('/api/support/wecom/getAuthToken', iPolloOSBaseURL);
...
- [iPolloOS Invoke Framework](../../../../../lib/invoke/README.md)
