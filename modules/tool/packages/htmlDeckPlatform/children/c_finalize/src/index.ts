...
export async function tool(props: In): Promise<Out> {
  try {
    ...
    const pageHtml = buildSingleFileHtml(state, { embedMermaid: inp.embed_mermaid });
    ...
    return {
      ...
      summary: `已生成最终幻灯片页面：${n} 页单文件 HTML（约 ${pageHtml.length} 字符）。${imageRequests.length ? `仍有 ${imageRequests.length} 个待生成配图，请先生成图片并回填 image_url。` : ''}这是最终 deliverable 的 page_html；自动发布后请向用户提供本工具产生的 page_url，不要把图表工具的 chart page_url 当作最终链接。`
    };
  } catch (e: unknown) {
