...
该插件内置 `nexu-io/html-anything` 的模板分类与运行增强能力。HTML 的生成由 FastGPT LangGraph / Agent 大脑完成，`html_anything_page` 工具只接收已经生成好的完整单文件 HTML，做校验、运行层注入和发布。
...
工具不会二次调用 AI 应用，也不需要 `HTML_ANYTHING_AI_APP_KEY`。如果 `content` 不是完整 HTML，工具会返回错误，LangGraph 大脑应重新生成完整 HTML 后再次调用。
