...
<main class="app">
  ...
  <section class="panel">
    <h1>${safeTitle}</h1>
    <p>V5_RUNTIME_BRIDGE：这是一个移动端功能应用工作台。iPolloOS AI 应用本次没有返回可发布 HTML，页面已自动降级为可操作版本，避免交付中断。</p>
  </section>
...
run.addEventListener('click',()=>{
  ...
  result.textContent='应用目标：'+value+'\\n\\n推荐交互：移动端工作台 + 分步骤任务流\\n\\n核心输入：用户目标、时间/素材/偏好、补充限制\\n\\n能力调用：页面动作应通过 iPolloOS Runtime 回到 AI 应用，由 AI 应用根据任务自动选择搜索、图像、视频、语音或数据库工具\\n\\n结果区：生成结果、可复制提示词、下一步优化建议、历史任务状态\\n\\n当前说明：iPolloOS AI 应用生成端返回失败，本页面为本地降级版本。修复上游后，可由 AI 自动生成更贴合需求的完整应用。';
});
</script>
</body>
</html>`;
