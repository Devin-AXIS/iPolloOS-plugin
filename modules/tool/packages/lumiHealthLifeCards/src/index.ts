...
const packageId = 'lumiHealthLifeCards';
const packageName = 'App 健康生活卡片';
...
export const InputType = z.object({
  card_type: z.enum(allowedCardTypes).default('daily_calorie'),
...
export async function tool(input: Input): Promise<Output> {
  try {
    ...
    const answer_text = [input.reply_text?.trim(), app_cards_comment].filter(Boolean).join('\n\n');
