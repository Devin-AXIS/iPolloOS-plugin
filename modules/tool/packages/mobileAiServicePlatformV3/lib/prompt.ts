const DEFAULT_CAPABILITIES =
  'ApolloOS/iPolloOS 应用已配置深图/图像生成与理解、深视频/视频生成与理解、网络搜索、工具调用与多步骤 Agent 编排能力。';
...
能力使用原则：
你正在调用的是一个已经配置好工具能力的 ApolloOS/iPolloOS 应用。开发这个移动端应用时，要把这些能力当成产品能力底座，而不是普通文案素材：
...
6. 必须善用 ApolloOS/iPolloOS 应用本身已经具备的 AI 与插件能力；不要生成一个与这些能力脱节的普通静态页面。
7. 必须开发功能性应用，而不是展示性页面：要有明确输入、操作、状态反馈、结果区域、异常/空状态和下一步动作；复杂需求要拆成合理的步骤或工作区。
8. 交互和应用完整度要做好：控件可用、按钮有响应、表单有校验或提示、结果能被查看/复制/继续优化；不要只堆文字说明。
9. 不要把真实 API Key 写入 HTML；如需表现接口调用，可用前端模拟状态、任务队列、结果卡片或提示词预览。
10. 允许内联 CSS 和少量原生 JS；不要依赖构建工具；外链资源必须可降级。
11. 返回给调用方时，最终答案必须包含完整 HTML；如果你的工作流有插件输出节点，优先把 HTML 放到 page_html 或 full_html 字段。`;
}
