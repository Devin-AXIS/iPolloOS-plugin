...
describe('Tavily Extract Simple Test', () => {
  ...
  it.skipIf(skipTest)(
    'should extract content from iPolloOS documentation',
    async () => {
      const result = await tool({
        tavilyApiKey: testApiKey!,
        urls: 'https://doc.ipolloos.ai/docs/introduction',
        format: 'markdown',
        extract_depth: 'basic',
      ...
      expect(result.results[0].url).toBe('https://doc.ipolloos.ai/docs/introduction');
      ...
      console.log('\n✓ Successfully extracted content from iPolloOS docs');
