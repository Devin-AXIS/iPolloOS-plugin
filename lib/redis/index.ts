...
export const IPOLLOOS_REDIS_PREFIX = 'ipolloos:';
export const getGlobalRedisConnection = () => {
  ...
  global.redisClient = new Redis(env.REDIS_URL, {
    ...REDIS_BASE_OPTION,
    keyPrefix: IPOLLOOS_REDIS_PREFIX
  });
...
export const getAllKeysByPrefix = async (key: string) => {
  ...
  const keys = (await redis.keys(`${IPOLLOOS_REDIS_PREFIX}${key}:*`)).map((key) =>
    key.replace(IPOLLOOS_REDIS_PREFIX, '')
  );
  return keys;
};
